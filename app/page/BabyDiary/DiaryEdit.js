...
class DiaryEdit extends Component {
  ...
  _keyboardDidShow(event) {
    ...
    if (that.refs.title.isFocused() == true) {
      var height1 =
        ScreenHeight - event.endCoordinates.height - NAVIGATION_HEIGHT;
  ...
  _save = () => {
    ...
    if (params.type == 0) {
      RealmManager.saveDiary({
        ...
        month: params.month + "",
        day: params.day + "",
        photos: this.state.assets == undefined ? [] : this.state.assets
      });
      goBack();
  ...
  _onScrollMove = () => {
    if (
      that.refs.content.isFocused() == false &&
    ...
    return (
      <Swipe
        ...
        style={[
          styles.swipe,
          {
            height: this.state.yOffset.interpolate({
              //映射到0.0,1.0之间
              inputRange: [
                -ScreenHeight,
                0,
      ...
      >
        <View style={styles.subcontent}>
          ...
          <TouchableOpacity
            ...
            onPress={this._onWeather}
          >
            <Image
              style={{ flex: 1, width: 25 }}
              resizeMode={"contain"}
              source={this.state.icon[this.state.currentWeatherIndex]}
            />
          </TouchableOpacity>
          <AutoExpandingTextInput
            placeholder={"写下你的今天..."}
  ...
  hud() {
    return (
      <KKInputHUD ref={"hud"} type={HUD.WEATHER} onPress={this._onHudClick} />
    );
  }
  ketaccess() {
    return (
